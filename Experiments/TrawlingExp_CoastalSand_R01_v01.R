...
# load baseline file
load("../Output/Baseline_CoastalSand.Rdata")
PL     <- ModelSandResults$PL
SV.ini <- ModelSandResults$output$y
...
sens.par$depth.mix <- c(1.3,2.3,3.4)
sens.par$t.res     <- c(1.5, 6.8, 30.)/365.25 
sens.par$depth.ero <- c(0.61,0.65,0.70)
...
for (i in 1:length(sens.par$frequency)){
  for (j in c(1,2,3)){
      ...
      #  Experiment name
      Run.name <- paste("TrawlingExp_CoastalSand_freq",sens.par$frequency[i],"perc",sens.par$name[j],sep="_")
